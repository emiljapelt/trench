...
let rec compile_value val_expr (state:compile_state) acc =
  ...
  | Binary_op (op, e1, e2) -> ( match op, type_value state e1, type_value state e2 with
    ...
    | ">>", T_Dir, T_Int -> Instr_Place :: I(8) :: (compile_value e1 state (compile_value e2 state (Instr_Add :: Instr_Mod :: acc)))
    | "<<", T_Dir, T_Int -> Instr_Place :: I(8) :: (compile_value e1 state (compile_value e2 state (Instr_Sub :: Instr_Mod :: acc)))
